...
app.post('/api/forgot', async (req, res) => {
    const { email, name, company, logo, generatedOtp } = req.body;
    ...
    try {
        ...
        const mailOptions = {
            from: process.env.EMAIL,
            to: user.email,
            subject: `${name} Password Reset OTP`,
                  ...
                  <table align="center" role="presentation" cellSpacing="0" cellPadding="0" border="0" width="100%" style="max-width:37.5em;margin-left:auto;margin-right:auto;margin-top:40px;margin-bottom:40px;width:465px;border-radius:0.25rem;border-width:1px;border-style:solid;border-color:rgb(234,234,234);padding:20px">
                     <tr style="width:100%">
                        <td>
                           ...
                           <h1 style="margin-left:0px;margin-right:0px;margin-top:30px;margin-bottom:30px;padding:0px;text-align:center;font-size:24px;font-weight:400;color:#01020A">Password Reset OTP</h1>
                           <p style="font-size:14px;line-height:24px;margin:16px 0;color:#01020A">Otp to reset the password for your account ${email} is ${generatedOtp}.</p>
...
//GET GENERATE THEORY
app.post('/api/generatecontinue', async (req, res) => {
    const receivedData = req.body;

    const promptString = receivedData.prompt;
    const storyHistory = receivedData.previous;
    ...
    await chat.sendMessage(prompt).then(result => {
        const response = result.response;
        const txt = response.text();
        const converter = new showdown.Converter();
        const markdownText = txt;
...
app.post('/api/course', async (req, res) => {
    const { user, content, descriptionPrompt, mainTopic, lang } = req.body;
    ...
    await model.generateContent(prompt).then(result => {
        ...
        const promptString = mainTopic + " illustration";
        ...
        async function logResults(error, results) {
            if (error) {
                ...
                const photo = results[0].url;
                try {
                    const newCourse = new Course({ user, content, type: text, mainTopic, photo, lang });
                    await newCourse.save();
                    res.json({ success: true, message: 'Story created successfully', courseId: newCourse._id, desc: text, photo: photo });
                } catch (error) {
                    res.status(500).json({ success: false, message: 'Internal server error' });
                }
            }
        ...
        const setSubscriptionPayload = (subscriptionPlanID) => {
            let subscriptionPayload = {
                ...
                "application_context": {
                    ...
                    "return_url": `https://successspacester.netlify.app`,
                    "cancel_url": `https://successspacester.netlify.app/fail`
                }
            }
...
app.post('/api/paypalupdate', async (req, res) => {
    ...
    try {
        const response = await fetch(`https://api-m.paypal.com/v1/billing/subscriptions/${id}/revise`, {
            ...
            body: JSON.stringify({ "plan_id": idPlan, "application_context": { "brand_name": process.env.COMPANY, "locale": "en-US", "payment_method": { "payer_selected": "PAYPAL", "payee_preferred": "IMMEDIATE_PAYMENT_REQUIRED" }, "return_url": `https://successspacester.netlify.app`, "cancel_url": `https://successspacester.netlify.app/fail` } })
        });
        const session = await response.json();
        res.send(session)
    } catch (error) {
