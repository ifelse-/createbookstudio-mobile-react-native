...
export default function SignUpPhone() {
    ...
    const handleSubmit = async () => {
        if (!isChecked) {
            ...
            if (email.length > 0 || mName.length > 0) {
                setLoad(true);
                const url = serverURL + '/api/signupphone';
                let type = 'free';
                let phone = params.mPhone;
                const response = await axios.post(url, {
                    email, mName, phone, type
                });
...
Thank you for joining us on this adventure. We look forward to hearing about the stories you discover and the worlds you explore with ${name}.</p>
    ...
    return (
        <View className="flex-1 bg-white" style={{ paddingTop: Constants.statusBarHeight + 10 }}>
            <BackIcon />
            <Text className="text-start text-base mt-6 mx-4" style={{ fontFamily: 'Montserrat_500Medium', color: '#01020A' }}>Enter Your Name & Email{'\n'}to Sign Up</Text>
            <View className="mt-4">
            ...
            <View>
                <Text className="text-start text-xs mt-4 mx-4" style={{ fontFamily: 'Montserrat_700Bold', color: '#01020A' }}>Email</Text>
